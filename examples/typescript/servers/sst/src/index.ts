...
// --- Environment variables (resolved at Lambda cold-start) ---
...
app.use("/*", async (c, next) => {
  ...
  console.log(`[x402] [${requestId}] --> ${method} ${path}`);
  console.log(`[x402] [${requestId}]     URL: ${url}`);
  console.log(
    `[x402] [${requestId}]     User-Agent: ${c.req.header("user-agent") ?? "(none)"}`,
  );
  console.log(
    `[x402] [${requestId}]     Accept: ${c.req.header("accept") ?? "(none)"}`,
  );
  ...
  if (paymentSig) {
    console.log(
      `[x402] [${requestId}]     PAYMENT-SIGNATURE: ${paymentSig}`,
    );
  } else if (xPayment) {
    console.log(`[x402] [${requestId}]     X-PAYMENT: ${xPayment}`);
  } else {
    console.log(`[x402] [${requestId}]     Payment header: (none)`);
  }
  ...
  try {
    ...
    console.error(
      `[x402] [${requestId}] !!! ${method} ${path} threw after ${elapsed}ms`,
    );
    console.error(
      `[x402] [${requestId}]     Error: ${err instanceof Error ? err.message : String(err)}`,
    );
    if (err instanceof Error && err.stack) {
      console.error(`[x402] [${requestId}]     Stack: ${err.stack}`);
    }
    throw err;
  }
  ...
  if (status === 402) {
    console.log(
      `[x402] [${requestId}] <-- ${status} Payment Required (${elapsed}ms)`,
    );
    const paymentRequired = c.res.headers.get("PAYMENT-REQUIRED");
    if (paymentRequired) {
      console.log(
        `[x402] [${requestId}]     PAYMENT-REQUIRED header set (${paymentRequired.length} chars)`,
      );
    }
  } else if (status >= 400) {
    console.warn(
      `[x402] [${requestId}] <-- ${status} ${method} ${path} (${elapsed}ms)`,
    );
  } else {
    console.log(`[x402] [${requestId}] <-- ${status} OK (${elapsed}ms)`);
  }

  const paymentResponse = c.res.headers.get("PAYMENT-RESPONSE");
  if (paymentResponse) {
    console.log(
      `[x402] [${requestId}]     PAYMENT-RESPONSE header set — settlement succeeded`,
    );
  }
});
...
<body>
  <div class="page">
    <a href="https://www.goplausible.com"><img class="logo-img" src="https://raw.githubusercontent.com/GoPlausible/.github/main/profile/assets/goPlausible-logo-type-h.png" alt="GoPlausible"></a>
    ...
    <div class="footer">
      <span class="footer-text">
        ...
        &middot; Read more about <a href="https://facilitator.goplausible.xyz">x402 on Algorand</a>
      </span>
...
app.get("/avm/weather", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving weather JSON`,
  );
  return c.json({ report: { weather: "sunny", temperature: 70 } });
});
app.get("/evm/weather", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving weather JSON`,
  );
  return c.json({ report: { weather: "sunny", temperature: 70 } });
});
app.get("/svm/weather", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving weather JSON`,
  );
  return c.json({ report: { weather: "sunny", temperature: 70 } });
...
<body>
  <div class="card">
    <a href="https://www.goplausible.xyz"><img class="logo-img" src="https://raw.githubusercontent.com/GoPlausible/.github/main/profile/assets/goPlausible-logo-type-h.png" alt="GoPlausible"></a>
    <div class="icon">&#x2713;</div>
    ...
    <div class="divider"></div>
    <p>
      This content is served by an <strong style="color:#e4e4e7">AWS Lambda</strong> function
      deployed with <strong style="color:#e4e4e7">SST</strong> using the
...
app.get("/avm/protected", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving protected HTML`,
  );
  return c.html(protectedHtml);
});
app.get("/evm/protected", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving protected HTML`,
  );
  return c.html(protectedHtml);
});
app.get("/svm/protected", c => {
  console.log(
    `[route] ${c.req.method} ${c.req.path} — payment verified, serving protected HTML`,
  );
  return c.html(protectedHtml);
