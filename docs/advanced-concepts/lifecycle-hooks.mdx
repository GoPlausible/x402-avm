...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    import { x402ResourceServer, x402HTTPResourceServer } from "@x402-avm/core";
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    import { x402Client } from "@x402-avm/core";
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    import { x402Client, x402HTTPClient } from "@x402-avm/core";
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript
    import { x402Facilitator } from "@x402-avm/core";
...
<Tabs>
  <Tab title="TypeScript">
    ```typescript server .onBeforeVerify(validatePayment)
    .onAfterVerify(logVerification) .onBeforeSettle(checkBalance)
    .onAfterSettle(recordTransaction); ```
  </Tab>
  <Tab title="Python">
    ```python ( server .on_before_verify(validate_payment)
    .on_after_verify(log_verification) .on_before_settle(check_balance)
    .on_after_settle(record_transaction) ) ```
  </Tab>
  <Tab title="Go">
    ```go server. OnBeforeVerify(validatePayment).
    OnAfterVerify(logVerification). OnBeforeSettle(checkBalance).
    OnAfterSettle(recordTransaction) ```
  </Tab>
</Tabs>
