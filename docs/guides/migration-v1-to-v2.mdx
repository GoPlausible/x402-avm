...
| Packages        | `x402`, `x402-express`, `x402-axios` | `@x402-avm/core`, `@x402-avm/express`, `@x402-avm/axios`, `@x402-avm/evm` |
...
<Tabs>
  <Tab title="TypeScript">
    1. **Package rename**: `x402-axios` → `@x402-avm/axios` 2. **Function
    rename**: `withPaymentInterceptor` → `wrapAxiosWithPayment` 3. **Wallet
    setup**: Use `x402Client` with `registerExactEvmScheme` helper instead of
    passing wallet directly 4. **No chain-specific configuration**: The V2
    client automatically handles network selection based on payment requirements
  </Tab>
  <Tab title="Python">
    1. **Import path changes**: `x402.clients.httpx` → `x402.http.clients` 2.
    **Signer wrapper**: Wrap `eth_account.Account` with `EthAccountSigner` 3.
    **Client construction**: Create `x402Client()` first, then register schemes
    4. **Environment variable**: `PRIVATE_KEY` → `EVM_PRIVATE_KEY` 5.
    **Async/Sync variants**: Use `x402Client` for httpx (async),
    `x402ClientSync` for requests (sync)
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="TypeScript">
    1. **Package rename**: `x402-express` → `@x402-avm/express` 2.
    **Configuration structure**: Route config now uses `accepts` array with
    explicit `scheme`, `network`, and `payTo` 3. **Network format**:
    `base-sepolia` → `eip155:84532` (CAIP-2 standard) 4. **Resource server**:
    Create `x402ResourceServer` with facilitator client and register schemes
    using helper functions 5. **Price recipient**: Explicitly specify `payTo`
    address per route
  </Tab>
  <Tab title="Python">
    1. **Import path changes**: `x402.fastapi.middleware` →
    `x402.http.middleware.fastapi` 2. **Middleware pattern**: `require_payment`
    decorator → `PaymentMiddlewareASGI` class 3. **Configuration structure**:
    Route config now uses `RouteConfig` and `PaymentOption` Pydantic models 4.
    **Network format**: `base-sepolia` → `eip155:84532` (CAIP-2 standard) 5.
    **Resource server**: Create `x402ResourceServer` and register schemes
    explicitly 6. **Type hints**: Use modern Python type hints (`dict[str, Any]`
    instead of `Dict[str, Any]`) 7. **Async/Sync variants**: Use
    `x402ResourceServer` + `HTTPFacilitatorClient` for FastAPI (async), use
    `x402ResourceServerSync` + `HTTPFacilitatorClientSync` for Flask (sync)
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="TypeScript">
    ...
    |------------|---------------|
    | `x402` | `@x402-avm/core` |
    | `x402-express` | `@x402-avm/express` |
    | `x402-axios` | `@x402-avm/axios` |
    | `x402-fetch` | `@x402-avm/fetch` |
    | `x402-hono` | `@x402-avm/hono` |
    | `x402-next` | `@x402-avm/next` |
    | (built-in) | `@x402-avm/evm` (EVM support) |
    | (built-in) | `@x402-avm/svm` (Solana support) |
  </Tab>
  <Tab title="Python">
    ...
    **Installation with extras:**
    ...
    pip install "x402-avm[httpx]"      # For async HTTP clients
    pip install "x402-avm[requests]"   # For sync HTTP clients
    pip install "x402-avm[fastapi]"    # For FastAPI servers
    pip install "x402-avm[flask]"      # For Flask servers
    pip install "x402-avm[svm]"        # For Solana support
    ```
...
<Tabs>
  <Tab title="TypeScript">
    ...
    ```bash
    # For buyers
    npm install @x402-avm/axios @x402-avm/evm

    # For sellers (Express)
    npm install @x402-avm/express @x402-avm/core @x402-avm/evm
    ```
